...
## 12. 遗留数据迁移与验证 (Legacy Data Migration & Verification)
### 12.1 数据源与 URL 模式验证
- **规则**: 在迁移涉及第三方资源（如图片 URL）的数据时，**必须**先行验证目标资源的 URL 模式。
- **反面教材**: 猜测图片 URL 为 `slug-based`（如 `/blue/front.png`），结果实际为 `id-based`（如 `/123456/front.png`），导致大量 404。
- **最佳实践**:
    1.  **先要在源头确认**: 在编写迁移脚本前，要求提供至少一个**可访问的**真实 URL 样本。
    2.  **不要假设一致性**: 前端代码 (`product-data.ts`) 可能与后端或存储桶实际结构不一致。检查是否有专门的 Seed 脚本 (`seed-color-mappings-v2.js`) 包含更完整的数据（如 `externalId`）。
    3.  **版本化 Console 脚本**: 提供给用户在浏览器控制台运行的脚本应包含明确的版本日志（如 `console.log('Starting Migration V3...')`）和校验步骤（写入后立即读取验证），防止用户误运行旧版本或未察觉写入失败。
