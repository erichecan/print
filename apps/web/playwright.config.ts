...
// 统一加载 E2E 环境变量（默认指向 configs/e2e.test.envvars）
const appRoot = __dirname;
const repoRoot = path.resolve(appRoot, '../..'); // 纠正 repo 根路径，指向 /print-main
const backendDir = path.resolve(repoRoot, 'backend');
const frontendDir = path.resolve(repoRoot, 'apps/web');
// 使用绝对路径，避免 webServer 在不同 CWD 下 cd 失败
...
const apiReadyUrl = `${apiURL.replace(/\/+$/, '')}/api/products?limit=1`; // 使用稳定 200 接口检测后端
...
// 允许通过 SKIP_WEB_SERVER=1 手动控制服务启动
// 如果本地服务器已运行，复用现有服务器
