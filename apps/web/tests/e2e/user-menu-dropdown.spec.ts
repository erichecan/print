/**
 * [2025-12-19] 用户菜单下拉框遮挡问题修复验证测试
 * 测试订单创建页面右上角头像下拉菜单是否正常显示且不被遮挡
 */
import { test, expect } from './fixtures/test-base';

// [2025-12-19] 使用与离线订单测试相同的测试账号
...
test.describe('用户菜单下拉框测试', () => {
  test('订单创建页面：登录后点击头像，下拉菜单应显示且不被遮挡', async ({ page }) => {
    ...
    // [2025-12-19] 2. 导航到订单创建页面
    await page.goto('/offline-orders');
    await page.waitForLoadState('networkidle');

    // [2025-12-19] 3. 等待页面加载完成，查找右上角的头像按钮
    // 头像按钮可能包含用户名字或SVG图标
    ...
    // [2025-12-19] 4. 验证头像按钮可见
    await expect(userMenuButton).toBeVisible({ timeout: 10000 });

    // [2025-12-19] 5. 点击头像按钮打开下拉菜单
    await userMenuButton.click();
    await page.waitForTimeout(500); // 等待菜单动画

    // [2025-12-19] 6. 验证下拉菜单是否显示
    ...
    // [2025-12-19] 9. 验证菜单定位方式：应该是fixed而不是absolute
