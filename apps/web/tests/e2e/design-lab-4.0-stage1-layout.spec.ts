 ...
 * [2025-12-19 23:56:00] 阶段1：验证4列3行布局结构，确保所有区域正确定位和尺寸
 */
...
test.describe('Design Lab 4.0 - 阶段1：布局结构验证', () => {
  test.beforeEach(async ({ page }) => {
    // [2025-12-19 23:56:00] 阶段1：导航到 Design Lab 页面
  ...
  test('阶段1-1：验证所有区域存在且可见', async ({ page }) => {
    // [2025-12-19 23:56:00] 阶段1：验证所有关键区域通过 data-testid 可被定位
  ...
  test('阶段1-2：验证 Header 高度为 64px', async ({ page }) => {
    // [2025-12-19 23:56:00] 阶段1：验证 Header 高度符合需求
  ...
  test('阶段1-3：验证 Rail 宽度为 80px', async ({ page }) => {
    // [2025-12-19 23:56:00] 阶段1：验证 Rail 宽度符合需求
  ...
  test('阶段1-4：验证 ToolPanel 宽度为 430px', async ({ page }) => {
    // [2025-12-19 23:56:00] 阶段1：验证 ToolPanel 宽度符合需求
  ...
  test('阶段1-5：验证 Sidebar 宽度为 120px', async ({ page }) => {
    // [2025-12-19 23:56:00] 阶段1：验证 Sidebar 宽度符合需求
  ...
  test('阶段1-6：验证 BottomBar 高度为 80px', async ({ page }) => {
    // [2025-12-19 23:56:00] 阶段1：验证 BottomBar 高度符合需求
  ...
  test('阶段1-7：验证布局为 4 列结构', async ({ page }) => {
    // [2025-12-19 23:56:00] 阶段1：验证 Main 区域内部是 4 列布局
  ...
  test('阶段1-8：验证布局为 3 行结构', async ({ page }) => {
    // [2025-12-19 23:56:00] 阶段1：验证外层容器是 3 行布局
  ...
  test('阶段1-9：验证 Canvas 区域可自适应', async ({ page }) => {
    // [2025-12-19 23:56:00] 阶段1：验证 Canvas 区域使用 1fr 自适应剩余空间
    const canvas = page.locator('[data-testid="canvas"]');
