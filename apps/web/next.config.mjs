...
const nextConfig = {
  ...
  async headers() {
    return [
      // HTML pages and API routes: never cache (content changes on each deploy)
      {
        source: '/:path*',
        headers: [
      ...
      // Next.js hashed static bundles: immutable for 1 year (filename changes on rebuild)
      {
      ...
      // Public folder assets: 1-day cache
      {
        source: '/images/:path*',
        headers: [
