...
* 服务端认证检查工具函数
* 修复：直接调用后端 API，而不是通过 Next.js API 路由
 */
...
* 修复：直接调用后端 API，使用 Authorization header 传递 token
* 修复：增强错误处理，避免因配置错误导致页面崩溃
 */
...
// 捕获 getBackendApiBaseUrl 可能抛出的错误
...
// 直接调用后端 API，使用 Authorization header 传递 token
