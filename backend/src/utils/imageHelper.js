...
* Helpers for generating optimized image URLs
 */
...
/**
* 将相对路径转换为完整的后端服务器URL
* 智能处理：自动检测环境，本地开发使用 localhost:3001，生产环境使用配置的 URL
* 修复：对于 /assets/ 路径（前端静态资源），保持相对路径或使用前端服务 URL
...
// 如果已经是完整的 URL（http:// 或 https://），检查是否是 GCS URL
...
// 对于前端静态资源路径（/assets/），优先检查是否已迁移到 GCS
  // 如果已迁移，数据库中的 URL 应该已经是完整的 GCS URL，这里保持原样
  if (url.startsWith('/assets/')) {
// 如果配置了 GCS，且 URL 可能是旧的静态资源路径，可以考虑映射
