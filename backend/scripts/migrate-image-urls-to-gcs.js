...
// 特殊处理商品图片路径：/assets/products/{slug}/{filename} -> product/{slug}/{filename}
...
// 迁移 ProductImage 表（使用 Prisma Client）
async function migrateProductImages() {
  const baseUrl = getImageBaseUrl();
console.log(' 🔍 开始迁移 商品图片 (product_images.url)');
  
  const images = await prisma.productImage.findMany({
    select: { id: true, url: true },
  });
  
console.log(` 在 product_images 中找到 ${images.length} 条有 URL 的记录`);
  ...
  for (const image of images) {
    ...
    if (DRY_RUN) {
console.log(` [DRY-RUN] 商品图片更新: ${original.substring(0, 80)}... -> ${mapped.substring(0, 80)}...`);
    } else {
...
console.log(` ✅ 商品图片迁移完成: 更新 ${updateCount} 条, 跳过 ${skipCount} 条`);
}

// 迁移 Variant 表
async function migrateVariants() {
  const baseUrl = getImageBaseUrl();
console.log(' 🔍 开始迁移 商品变体图片 (variants.imageUrl)');
  
  const variants = await prisma.variant.findMany({
    select: { id: true, imageUrl: true },
  });
  
console.log(` 在 variants 中找到 ${variants.length} 条有 imageUrl 的记录`);
  ...
  for (const variant of variants) {
    ...
    if (DRY_RUN) {
console.log(` [DRY-RUN] 变体图片更新: ${original?.substring(0, 80)}... -> ${mapped?.substring(0, 80)}...`);
    } else {
...
console.log(` ✅ 变体图片迁移完成: 更新 ${updateCount} 条, 跳过 ${skipCount} 条`);
}

// 迁移 Category 表
async function migrateCategories() {
  const baseUrl = getImageBaseUrl();
console.log(' 🔍 开始迁移 分类图片 (categories.imageUrl)');
  
  const categories = await prisma.category.findMany({
    select: { id: true, imageUrl: true },
  });
  
console.log(` 在 categories 中找到 ${categories.length} 条有 imageUrl 的记录`);
  ...
  for (const category of categories) {
    ...
    if (DRY_RUN) {
console.log(` [DRY-RUN] 分类图片更新: ${original?.substring(0, 80)}... -> ${mapped?.substring(0, 80)}...`);
    } else {
...
console.log(` ✅ 分类图片迁移完成: 更新 ${updateCount} 条, 跳过 ${skipCount} 条`);
}

// 迁移 Brand 表
async function migrateBrands() {
  const baseUrl = getImageBaseUrl();
console.log(' 🔍 开始迁移 品牌 Logo (brands.logoUrl)');
  
  const brands = await prisma.brand.findMany({
    select: { id: true, logoUrl: true },
  });
  
console.log(` 在 brands 中找到 ${brands.length} 条有 logoUrl 的记录`);
  ...
  for (const brand of brands) {
    ...
    if (DRY_RUN) {
console.log(` [DRY-RUN] 品牌 Logo 更新: ${original?.substring(0, 80)}... -> ${mapped?.substring(0, 80)}...`);
    } else {
...
console.log(` ✅ 品牌 Logo 迁移完成: 更新 ${updateCount} 条, 跳过 ${skipCount} 条`);
}

// 迁移 Promotion 表
async function migratePromotions() {
  const baseUrl = getImageBaseUrl();
console.log(' 🔍 开始迁移 促销 Banner (promotions.bannerImageUrl)');
  
  const promotions = await prisma.promotion.findMany({
    select: { id: true, bannerImageUrl: true },
  });
  
console.log(` 在 promotions 中找到 ${promotions.length} 条有 bannerImageUrl 的记录`);
  ...
  for (const promotion of promotions) {
    ...
    if (DRY_RUN) {
console.log(` [DRY-RUN] 促销 Banner 更新: ${original?.substring(0, 80)}... -> ${mapped?.substring(0, 80)}...`);
    } else {
...
console.log(` ✅ 促销 Banner 迁移完成: 更新 ${updateCount} 条, 跳过 ${skipCount} 条`);
}

// 迁移 Design 表
async function migrateDesigns() {
  const baseUrl = getImageBaseUrl();
console.log(' 🔍 开始迁移 设计缩略图 (designs.thumbnailUrl)');
  
  const designs = await prisma.design.findMany({
    select: { id: true, thumbnailUrl: true },
  });
  
console.log(` 在 designs 中找到 ${designs.length} 条有 thumbnailUrl 的记录`);
  ...
  for (const design of designs) {
    ...
    if (DRY_RUN) {
console.log(` [DRY-RUN] 设计缩略图更新: ${original?.substring(0, 80)}... -> ${mapped?.substring(0, 80)}...`);
    } else {
...
console.log(` ✅ 设计缩略图迁移完成: 更新 ${updateCount} 条, 跳过 ${skipCount} 条`);
}

// 迁移 DesignAsset 表
async function migrateDesignAssets() {
  const baseUrl = getImageBaseUrl();
console.log(' 🔍 开始迁移 设计素材 (design_assets.url)');
  
  const assets = await prisma.designAsset.findMany({
    select: { id: true, url: true },
  });
  
console.log(` 在 design_assets 中找到 ${assets.length} 条有 url 的记录`);
  ...
  for (const asset of assets) {
    ...
    if (DRY_RUN) {
console.log(` [DRY-RUN] 设计素材更新: ${original?.substring(0, 80)}... -> ${mapped?.substring(0, 80)}...`);
    } else {
...
console.log(` ✅ 设计素材迁移完成: 更新 ${updateCount} 条, 跳过 ${skipCount} 条`);
}

async function main() {
  console.log(
` 🚀 开始执行图片 URL -> GCS 迁移脚本 (DRY_RUN=${DRY_RUN})`
  );

  try {
    const baseUrl = getImageBaseUrl();
    console.log(
` 使用的 GCS 基础 URL: ${baseUrl}`
    );

// 使用 Prisma Client 迁移各个表
    await migrateProductImages();
    
// 其他表的迁移（商品图片是主要目标，其他表可选）
    try {
      await migrateCategories();
    } catch (e) {
console.log(` ⚠️ 跳过分类图片迁移: ${e.message}`);
    }
    
    try {
      await migrateBrands();
    } catch (e) {
console.log(` ⚠️ 跳过品牌 Logo 迁移: ${e.message}`);
    }
    
// 跳过 Variant、Promotion、Design 等表的迁移（这些表可能没有图片数据或 Prisma 模型不可用）
console.log(' ℹ️ 跳过变体、促销、设计等图片迁移（主要商品图片已完成）');

    console.log(
` 🎉 图片 URL 迁移脚本执行完毕 (DRY_RUN=${DRY_RUN})`
    );
  } catch (error) {
    console.error(
' ❌ 图片 URL 迁移脚本失败:',
