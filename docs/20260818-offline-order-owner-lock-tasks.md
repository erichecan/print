...
- [ ] T1 后端：`utils/offlineOrderOwnership.js`（归属判定 + 两个中间件）；`mapOrder` 暴露 `creatorId`
...
- [ ] T7 验证：前端 build + 后端鉴权冒烟
      验收：npm run build 通过；403 用例实测
